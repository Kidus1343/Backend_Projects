...
// Enhanced type declarations for Pactum
declare module 'pactum' {
  interface Spec {
    // HTTP Methods
    get(url: string): Spec;
    ...
    withHeaders(headers: Record<string, string>): Spec;
    withPathParams(key: string, value: string): Spec;
    withQueryParams(key: string, value: string): Spec;

    // Assertions
    expectStatus(code: number, message?: string): Spec;
    expectBody(body: any): Spec;
    expectBodyContains(text: string): Spec;
    expectJsonLength(length: number): Spec;
    expectJsonMatch(json: any): Spec;

    // Stores
    stores(key: string, path: string): Spec;
  }
}
